...
class Strategy:
    ...
    def index_arb1(
        ...
        underlying_prc = 6 * croissants.vwap + 3 * jams.vwap + 1 * djembe.vwap
    ...
    def index_arb2(
        ...
        basket_prc = basket.mid
        underlying_prc = 4 * croissants.vwap + 2 * jams.vwap
        spread = basket_prc - underlying_prc
        norm_spread = spread - theta
...
class Trade:
    ...
    def rainforest_resin(state: Status) -> list[Order]:
        ...
        orders.extend(Strategy.mm_ou(state=state, fair_price=current_price, gamma=0.1, order_amount=Status._position_limit["RAINFOREST_RESIN"]))
    ...
    def kelp(state: Status) -> list[Order]:
        ...
        orders = []
        orders.extend(Strategy.arb(state=state, fair_price=current_price))
        orders.extend(Strategy.mm_ou(state=state, fair_price=current_price, gamma=0.1, order_amount=Status._position_limit["KELP"]))
    ...
    def squid_ink(state: Status) -> list[Order]:
        ...
        orders = []
        # orders.extend(Strategy.arb(state=state, fair_price=current_price))
        orders.extend(Strategy.mm_glft(state=state, fair_price=current_price, gamma=0.1, order_amount=Status._position_limit["SQUID_INK"]))

        return orders
    
    @staticmethod
    def picnic_basket1(basket: Status, croissants: Status, jams: Status, djembes: Status) -> list[Order]:

        orders = []
        orders.extend(Strategy.index_arb1(basket, croissants, jams, djembes, threshold=84, theta=49))

        return orders

    @staticmethod
    def picnic_basket2(basket: Status, croissants: Status, jams: Status) -> list[Order]:
